 ...
 * Drops and recreates the SQLite database from db/schema.sql, then inserts
 * fictional demo data. All names/emails are obviously fake (@example.com).
 */
...
const riskScores = [12, 85, 34, 71, 22, 90, 45, 8, 76, 51, 29, 67, 95, 18, 40, 73, 55, 31, 88, 26, 62, 14, 79, 48, 37];

const cases: SeedCase[] = names.map((n, i) => ({
...
for (let i = 12; i < 17; i++) {
  cases[i].status = "in_review";
  cases[i].assigned_to = reviewers[i % 3];
}
...
const insCase = db.prepare(`INSERT INTO kyc_cases
...
insNote.run("note-1", "case-013", "u-maria", "Waiting on customer to re-upload a clearer passport photo page.", daysAgo(4));
insNote.run("note-2", "case-014", "u-james", "Address on file differs from document; requested proof of address.", daysAgo(3));
insNote.run("note-3", "case-023", "u-james", "Tampering suspicion confirmed by second reviewer before rejection.", daysAgo(2));
insNote.run("note-4", "case-016", "u-priya", "High risk score driven by geography; no adverse media found so far.", daysAgo(1));

// --- coherent audit_log + access_log entries for decided cases -----------------
...
// --- feature flags (stretch app) -----------------------------------------------
const insFlag = db.prepare(
  "INSERT INTO feature_flags (id, key, description, enabled, environment, updated_at) VALUES (?, ?, ?, ?, ?, ?)"
);
insFlag.run("flag-1", "new-onboarding-flow", "Redesigned customer onboarding wizard", 1, "staging", daysAgo(10));
insFlag.run("flag-2", "new-onboarding-flow-prod", "Redesigned customer onboarding wizard (production rollout)", 0, "production", daysAgo(9));
