...
```bash
npm install
npm run seed        # recreates db/dev.sqlite (4 users, 25 cases, 4 flags)
...
- `npm run seed` may fail with `SQLITE_IOERR_SHORT_READ` if the dev server holds the DB's WAL files. Kill the dev server and `rm -f db/dev.sqlite*` before reseeding.
