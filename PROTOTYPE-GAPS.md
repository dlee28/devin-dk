...
| Linked databases are local SQLite files | Relinking an app really repoints its queries (`platform/databases.ts` resolves `linked_database` to `db/data/<name>.sqlite`), and admins can link/remove databases, but they are unauthenticated files switched in-process | Credentialed connections (per-env hosts, secrets manager), migration tooling, and connection pooling per database | 1-2 engineer-weeks |
