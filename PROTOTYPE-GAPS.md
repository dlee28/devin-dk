...
| Linked database is a label | The per-app settings page stores which database an app is linked to, but the prototype has one SQLite file, so switching the value doesn't repoint queries | Per-app connection registry resolving the setting to real, credentialed connections per environment | 1 engineer-week |
